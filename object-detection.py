...
def load_local_detr_pipeline(local_model_path):
    ...
    try:
        ...
        # Local snapshot may not contain preprocessor_config.json.
        image_processor = DetrImageProcessor()

    return pipeline("object-detection", model=model, image_processor=image_processor)


# innotator = pipeline("text-to-speech", model="kakao-enterprise/vits-ljs")
narrator = pipeline("text-to-speech", model=innotator_path)
object_detector = load_local_detr_pipeline(model_path)
...
def generate_audio(text):
    ...
    wavefile.write("output.wav", rate=narrated_text["sampling_rate"],
